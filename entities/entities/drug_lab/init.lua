...
function ENT:Use(activator,caller)
	...
	if activator.maxDrugs and activator.maxDrugs >= GAMEMODE.Config.maxdrugs then
		GAMEMODE:Notify(activator, 1, 3, "You can't make anymore Pills as the limit is reached.")
		...
		if not activator:CanAfford(productioncost) then
			GAMEMODE:Notify(activator, 1, 4, "You do not have enough money to produce Pills.")
			return false
		end
		activator:AddMoney(-productioncost)
		GAMEMODE:Notify(activator, 0, 4, "You have made Pills! production cost: " .. CUR .. tostring(productioncost).."!")
		self.sparking = true
