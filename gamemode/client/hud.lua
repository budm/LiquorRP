...
function LDRP.MoreInfoToggle()
	...
	if !timer.IsTimer("ReSizeInfoWindow") then
...
function LDRP.HUDPaint()
	...
	if !OptionsButton then
		...
		OptionsButton:SetMaterial( "gui/silkicons/wrench" )
...
if LDRP_SH.ReplaceSkybox then
	...
	function mat.ReplaceSkybox(new)
		for k,v in pairs(mat.SkyNames) do
			local D = Material("skybox/".. new .. v):GetMaterialTexture("$basetexture")
			mat.CurrentMats[k]:SetMaterialTexture("$basetexture",D)
		end
	end
...
function LDRP_SH.CancelMeter()
	if timer.IsTimer("MeterHUDRemove") then
		hook.Remove("HUDPaint","Shows a progress meter")
...
if !UseFadmin then
	...
	function LDRP.Scoreboard()
		...
		function B.CreateBlur(self)
			...
			local x1, y1 = self:LocalToScreen( 2, 2 )
			local x2, y2 = self:LocalToScreen( W - 2, H - 2 )
			x1 = x1 + 5 y1 = y1 + 5
			x2 = x2 - 5 y2 = y2 - 5
			
			matBlurScreen:SetMaterialFloat( "$blur", 6 )
			render.UpdateScreenEffectTexture()
			surface.DrawPoly( {
				{ x = 2, y = 2, u = x1/ScrW(), v = y1/ScrH() },
				{ x = W - 4, y = 2, u = x2/ScrW(), v = y1/ScrH() },
				{ x = W - 4, y = H - 2, u = x2/ScrW(), v = y2/ScrH() },
				{ x = 2, y = H - 2, u = x1/ScrW(), v = y2/ScrH() }
			} )
		end
		...
		B.BG.Paint = function(self)
			...
			--draw.RoundedBox(0,12,54,876,534,Color(150,150,150,160))
			--B.CreateBlur(
		end
		B.BG:SetTitle("")
		...
		for k,v in pairs(player.GetAll()) do
			...
			Ply.Think = function() if !v:IsValid() then Ply:Remove() end end
			Ply.Paint = function(self)
			...
			timer.Simple(0,function()
				...
				Info:SetImage("gui/silkicons/page")
